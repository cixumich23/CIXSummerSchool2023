...
def assign(gui_elements, current_task, max_capacity, num_slots):
    ...
    item_tuples = []
    for i in range(0, len(gui_elements)):
        for j in range(0, len(gui_elements)):
            item_tuples.append((gui_elements[i].name, gui_elements[j].name))

    pairwise_similarities = {(item1, item2) : random.random() for item1, item2 in item_tuples}
    ...
    model.setObjective(gp.quicksum(pairwise_similarities[(item1, item2)] * assign[item1] * assign[item2] for item1, item2 in item_tuples))
